...
// Default initial viewport for the service region (Hyderabad)
...
// OpenStreetMap tiles are used with Leaflet in embedded WebView
const MAP_HTML = `
<!DOCTYPE html>
<html>
  <head>
...
function titleForPhase(phase: LocationPhase, location: CustomerLocation | null): string {
  if (phase === 'locating') return 'Finding your GPS location...';
  if (phase === 'resolving' && !location?.address) return 'Detecting address...';
...
function subtitleForPhase(
  ...
  if (phase === 'locating') return 'Fetching high-accuracy coordinates...';
  if (phase === 'resolving' && !location?.address) return 'Checking the address and pickup availability...';
  ...
  return [...placeParts, location?.pincode].filter(Boolean).join(' - ') || 'Drag map or search to adjust.';
}
...
export function MapLocationPickerScreen({
  ...
  // Reverse geocode initial coordinates on mount if address is not already known
  useEffect(() => {
    if (initialResolvedRef.current) return;
    initialResolvedRef.current = true;
    if (!initialLocation?.address) {
      void resolveSelection(coords.latitude, coords.longitude, 'manual');
    }
  }, [coords.latitude, coords.longitude, initialLocation?.address, resolveSelection]);
  ...
  const handleConfirm = useCallback(async () => {
    const locToSave = resolvedLocation || {
      address: `Selected Location (${coords.latitude.toFixed(4)}, ${coords.longitude.toFixed(4)})`,
      formattedAddress: `Selected Location (${coords.latitude.toFixed(4)}, ${coords.longitude.toFixed(4)})`,
    ...
    try {
      if (customerId && (houseNo.trim() || addressTag)) {
        // Persist to backend customer addresses
        const addrPayload: any = {
          id: `addr_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
          type: addressTag === 'Work' ? 'Office' : addressTag,
          houseNo: houseNo.trim(),
          street: locToSave.street || locToSave.address || 'Selected Location',
          area: locToSave.areaName || locToSave.locality || '',
          landmark: landmark.trim(),
          city: locToSave.city || 'Hyderabad',
          pincode: locToSave.pincode || '500085',
          latitude: coords.latitude,
          longitude: coords.longitude,
          isDefault: true,
        };
        await api.addAddress(customerId, addrPayload).catch(() => {});
      }
      await onLocationConfirmed(finalLocation);
    } finally {
      setIsSubmitting(false);
    }
  }, [addressTag, coords.latitude, coords.longitude, customerId, houseNo, landmark, onLocationConfirmed, resolvedLocation, selectionSource]);
  ...
  const canUseLocation = Boolean(resolvedLocation) || Boolean(coords);
  ...
  const serviceability = phase === 'locating'
    ...
    : resolvedLocation?.isServiceable === true
      ...
      : resolvedLocation?.isServiceable === false
        ? { state: 'unavailable' as const, text: `Area detected (PIN ${resolvedLocation.pincode || 'detected'}). Serviceability verified at checkout.` }
        : null;

  const actionLabel = isSubmitting ? 'Saving location...' : 'Save & Use This Location';
...
const styles = StyleSheet.create({
  ...
  mapArea: { flex: 1, minHeight: 140, backgroundColor: '#E5E7EB', overflow: 'hidden' },
  ...
  addressRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 10 },
  ...
  serviceabilityRow: { flexDirection: 'row', alignItems: 'center', gap: 7, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 8, marginBottom: 10 },
  ...
  searchLink: { minHeight: 34, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, marginTop: 2, marginBottom: 2 },
  searchLinkText: { color: '#2563EB', fontSize: 12.5, fontWeight: '700' },
  searchPanel: { marginTop: 4, marginBottom: 8 },
  searchInputRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
